...
void svt_cdef_row(EbDecHandle *dec_handle, int32_t *mi_wide_l2,
    int32_t *mi_high_l2, uint16_t **colbuf,
    uint8_t *prev_row_cdef, uint8_t *curr_row_cdef,
    int32_t fbr, uint16_t *src, int32_t *curr_recon_stride,
    uint8_t **curr_blk_recon_buf, uint16_t **line_buf)
{
    ...
    for (int32_t fbc = 0; fbc < nhfb; fbc++) {
        svt_cdef_block(dec_handle, mi_wide_l2, mi_high_l2, colbuf,
            prev_row_cdef, curr_row_cdef, fbr, fbc, &cdef_left, num_planes,
            src, curr_recon_stride, curr_blk_recon_buf, line_buf);
    }
}
#endif
...
#if MT_SUPPORT
        svt_cdef_row(dec_handle, &mi_wide_l2[0], &mi_high_l2[0], &colbuf[0],
            prev_row_cdef, curr_row_cdef, fbr, &src[0], &curr_recon_stride[0],
            &curr_blk_recon_buf[0], &linebuf[0]);
