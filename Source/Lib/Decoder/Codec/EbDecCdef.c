...
void svt_cdef_block(EbDecHandle *dec_handle, int32_t *mi_wide_l2,
    int32_t *mi_high_l2, uint16_t **colbuf, uint8_t *prev_row_cdef,
    uint8_t *curr_row_cdef, int32_t fbr, int32_t fbc,
    uint32_t *cdef_left, int32_t num_planes, uint16_t *src,
    ...
    for (int32_t pli = 0; pli < num_planes; pli++) {
        ...
        if (use_highbd)
            copy_sb16_16(
                ...
                CDEF_BSTRIDE, (uint16_t *)rec_buff/*xd->plane[pli].dst.buf*/,
        ...
        if (!prev_row_cdef[fbc - 1]) {
            if (use_highbd)
                copy_sb16_16(//cm,
                    src, CDEF_BSTRIDE, (uint16_t *)rec_buff/*xd->plane[pli].dst.buf*/,
                    (MI_SIZE_64X64 << mi_high_l2[pli])*fbr - CDEF_VBORDER,
        ...
        if (!prev_row_cdef[fbc + 1]) {
            ...
            copy_rect(&src[hsize + CDEF_HBORDER], CDEF_BSTRIDE,
                &linebuf_above[pli][coffset + hsize], stride, CDEF_VBORDER,
                CDEF_HBORDER);
        }
        else {
        ...
        if (fbr < nvfb - 1) {
            if (use_highbd)
                copy_sb16_16(&linebuf_curr[pli][coffset], stride, (uint16_t *)rec_buff,
                (MI_SIZE_64X64 << mi_high_l2[pli]) *
                    (fbr + 1) - CDEF_VBORDER,
                    coffset, rec_stride, CDEF_VBORDER, hsize);
            else
                copy_sb8_16(&linebuf_curr[pli][coffset], stride, rec_buff,
                (MI_SIZE_64X64 << mi_high_l2[pli]) *
                    (fbr + 1) - CDEF_VBORDER,
                    coffset, rec_stride, CDEF_VBORDER, hsize);
        }
    ...
    uint8_t **curr_blk_recon_buf)
{
    ...
    int32_t picture_width_in_sb =
        (dec_handle->seq_header.max_frame_width + sb_size_w - 1) / sb_size_w;
    ...
    for (int32_t pli = 0; pli < num_planes; pli++) {
        ...
        if (sb_128) {
            fill_rect(colbuf[pli + 3], CDEF_HBORDER, block_height, CDEF_HBORDER,
                CDEF_VERY_LARGE);
    ...
    volatile uint32_t *cdef_completed_in_prev_row;
    ...
    if (sb_fbr) {
        cdef_completed_in_prev_row = (volatile uint32_t *)
            &dec_mt_frame_data->cdef_completed_in_row[sb_fbr - 1];
    }
    cdef_completed_in_row =
        &dec_mt_frame_data->cdef_completed_in_row[sb_fbr];
    ...
    int32_t cdef_left_64[2] = { 1,1 };
    uint8_t *curr_row_cdef[2], *prev_row_cdef[2];

    curr_row_cdef[0] = curr_row_cdef_map +
        ((sb_fbr << sb_64_shift) * cdef_map_stride);
    ...
    /*for loop of SB cols*/
    for (int32_t sb_fbc = 0; sb_fbc < picture_width_in_sb; sb_fbc++) {

        /* Top-Right Sync*/
        if (sb_fbr) {
            if (sb_fbc == picture_width_in_sb - 1)
                nsync = 0;
        ...
        if (0 == sb_fbc && sb_128) {
            ...
            fbc_64 = row_cnt;
            svt_cdef_block(dec_handle, mi_wide_l2, mi_high_l2,
                colbuf_64[row_cnt],
                prev_row_cdef[row_cnt], curr_row_cdef[row_cnt],
                fbr_64, fbc_64,
                &cdef_left_64[row_cnt], num_planes,
                src, curr_recon_stride, curr_blk_recon_buf,
                dec_mt_frame_data->cdef_linebuf[fbr_64],/*above*/
                dec_mt_frame_data->cdef_linebuf[min(fbr_64+1, nvfb-1)],/*current*/
        ...
        for (int32_t cnt = 0; cnt < cnt_64; cnt++) {
            ...
            fbr_64 = (sb_fbr << sb_64_shift) + (row_cnt);
            fbc_64 = (sb_fbc << sb_64_shift) + (cnt & 1) +
                (sb_128 ? !row_cnt : 0);
    ...
    for (int32_t fbr = 0; fbr < nvfb; fbr++) {
        ...
        for (int32_t fbc = 0; fbc < nhfb; fbc++) {
            ...
            for (int32_t pli = 0; pli < num_planes; pli++) {
                ...
                if (fbr < nvfb - 1) {
                    if (use_highbd)
                        copy_sb16_16(&linebuf[pli][coffset], stride, (uint16_t *)rec_buff,
                            (MI_SIZE_64X64 << mi_high_l2[pli]) *
                            (fbr + 1) - CDEF_VBORDER,
                            coffset, rec_stride, CDEF_VBORDER, hsize);
                    else
