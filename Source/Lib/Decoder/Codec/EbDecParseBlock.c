    ...
    int mi_row, int mi_col, SvtReader *r, int8_t *cdef_strength)
{
    ...
    IntMv_dec ref_mvs[INTRA_FRAME + 1][MAX_MV_REF_CANDIDATES] = { { { 0 } } };
...
void mode_info(EbDecHandle *decHandle, PartitionInfo_t *part_info,
    uint32_t mi_row, uint32_t mi_col, SvtReader *r, int8_t *cdef_strength)
{
    ...
    if (frame_info->frame_type == KEY_FRAME ||
        frame_info->frame_type == INTRA_ONLY_FRAME)
    {
        intra_frame_mode_info(decHandle, part_info, mi_row,
                              mi_col, r, cdef_strength);
        intra_copy_frame_mvs(decHandle, mi_row, mi_col, x_mis, y_mis);
    }
    else {
        inter_frame_mode_info(decHandle, part_info, mi_row,
                              mi_col, r, cdef_strength);
