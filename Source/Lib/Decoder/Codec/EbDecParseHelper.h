...
int txfm_partition_context(TXFM_CONTEXT *above_ctx, TXFM_CONTEXT *left_ctx, block_size bsize, TxSize tx_size);
...
int dec_is_chroma_reference(int mi_row, int mi_col, block_size bsize,  int subsampling_x, int subsampling_y);
UV_PredictionMode read_intra_mode_uv(FRAME_CONTEXT *ec_ctx, SvtReader *r, CFL_ALLOWED_TYPE cfl_allowed, PredictionMode y_mode);
CFL_ALLOWED_TYPE is_cfl_allowed(PartitionInfo_t *xd, EbColorConfig* color_cfg, uint8_t *lossless_array);
int allow_palette(int allow_screen_content_tools, block_size sb_type);
...
void clamp_mv(MV_dec *mv, int min_col, int max_col, int min_row, int max_row);
PredictionMode dec_get_uv_mode(UV_PredictionMode mode);
TxType intra_mode_to_tx_type(const ModeInfo_t *mbmi, PLANE_TYPE plane_type);
int has_second_ref(const ModeInfo_t *mbmi);
IntMv_dec gm_get_motion_vector(const EbWarpedMotionParams *gm, int allow_hp, block_size bsize, int mi_col, int mi_row, int is_integer);
...
aom_cdf_prob *get_y_mode_cdf(FRAME_CONTEXT *tile_ctx, const ModeInfo_t *above_mi, const ModeInfo_t *left_mi);

int get_comp_index_context();
