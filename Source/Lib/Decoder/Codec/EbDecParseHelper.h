...
int txfm_partition_context(TXFM_CONTEXT *above_ctx, TXFM_CONTEXT *left_ctx, BlockSize bsize, TxSize tx_size);
...
int dec_is_chroma_reference(int mi_row, int mi_col, BlockSize bsize,  int subsampling_x, int subsampling_y);
UvPredictionMode read_intra_mode_uv(FRAME_CONTEXT *ec_ctx, SvtReader *r, CflAllowedType cfl_allowed, PredictionMode y_mode);
CflAllowedType is_cfl_allowed(PartitionInfo_t *xd, EbColorConfig* color_cfg, uint8_t *lossless_array);
...
IntMv_dec gm_get_motion_vector(const EbWarpedMotionParams *gm, int allow_hp, BlockSize bsize, int mi_col, int mi_row, int is_integer);
...
AomCdfProb *get_y_mode_cdf(FRAME_CONTEXT *tile_ctx, const ModeInfo_t *above_mi, const ModeInfo_t *left_mi);
