...
static void scan_col_mbmi(EbDecHandle *dec_handle, ParseCtxt *parse_ctx,
    PartitionInfo_t *pi, int delta_col, int mi_row, int mi_col,
    const MvReferenceFrame rf[2], CandidateMv *ref_mv_stack,
    uint8_t *num_mv_found, uint8_t *found_match, uint8_t *newmv_count,
...
static void scan_blk_mbmi(EbDecHandle *dec_handle, ParseCtxt* parse_ctx,
    PartitionInfo_t *pi, int delta_row, int delta_col, const int mi_row,
    const int mi_col, const MvReferenceFrame rf[2], CandidateMv *ref_mv_stack,
...
static int add_tpl_ref_mv(EbDecHandle *dec_handle, ParseCtxt *parse_ctx,
    int mi_row, int mi_col, MvReferenceFrame ref_frame, int blk_row, int blk_col,
    IntMv *gm_mv_candidates, uint8_t *num_mv_found,
    CandidateMv ref_mv_stacks[][MAX_REF_MV_STACK_SIZE], int16_t *mode_context)
{
    uint8_t idx;
    FrameHeader *frm_header = parse_ctx->frame_header;
    ...
    if (rf[1] == NONE_FRAME) {
        ...
        int cur_offset_0 = get_relative_dist(&parse_ctx->seq_header->order_hint_info,
        ...
        int cur_offset_1 = get_relative_dist(&parse_ctx->seq_header->order_hint_info,
...
static void add_extra_mv_candidate(BlockModeInfo * candidate, ParseCtxt *parse_ctx,
...
static void process_single_ref_mv_candidate(BlockModeInfo * candidate,
    ParseCtxt *parse_ctx, MvReferenceFrame ref_frame,
    ...
    int mi_row, int mi_col, int16_t *mode_context, MvCount *mv_cnt)
{
    ...
    if (abs(max_row_offset) >= 1) {
        scan_row_mbmi(dec_handle, parse_ctx, pi, -1, mi_row, mi_col, rf, ref_mv_stack[ref_frame],
    ...
    if (abs(max_col_offset) >= 1) {
        scan_col_mbmi(dec_handle, parse_ctx, pi, -1, mi_row, mi_col, rf, ref_mv_stack[ref_frame],
    ...
    if (has_top_right(dec_handle, pi, mi_row, mi_col, bs)) {
        scan_blk_mbmi(dec_handle, parse_ctx, pi, -1, n4_w, mi_row, mi_col, rf,
    ...
    if (frame_info->use_ref_frame_mvs) {
        ...
        for (int blk_row = 0; blk_row < blk_row_end; blk_row += step_h) {
            for (int blk_col = 0; blk_col < blk_col_end; blk_col += step_w) {
                ...
                int ret = add_tpl_ref_mv(dec_handle, parse_ctx, mi_row, mi_col,
        ...
        if (allow_extension) {
            for (int i = 0; i < 3; ++i) {
                ...
                if (check_sb_border(mi_row, mi_col, blk_row, blk_col)) {
                    add_tpl_ref_mv(dec_handle, parse_ctx, mi_row, mi_col, ref_frame, blk_row,
    ...
    scan_blk_mbmi(dec_handle, parse_ctx, pi, -1, -1, mi_row, mi_col, rf,
        ref_mv_stack[ref_frame], &mv_cnt->found_above_match, &mv_cnt->newmv_count,
        gm_mv_candidates, &mv_cnt->num_mv_found[ref_frame]);
    ...
    for (int idx = 2; idx <= MVREF_ROW_COLS; ++idx) {
        ...
        if (abs(row_offset) <= abs(max_row_offset) && abs(row_offset) > processed_rows) {
            scan_row_mbmi(dec_handle, parse_ctx, pi, row_offset, mi_row, mi_col, rf,
        ...
        if (abs(col_offset) <= abs(max_col_offset) && abs(col_offset) > processed_cols) {
            scan_col_mbmi(dec_handle, parse_ctx, pi, col_offset, mi_row, mi_col, rf,
                ref_mv_stack[ref_frame], &mv_cnt->num_mv_found[ref_frame],
    ...
    if (mv_cnt->num_mv_found[ref_frame] < MAX_MV_REF_CANDIDATES) {
        ...
        for (int pass = 0; pass < 2; pass++) {
                ...
                mv_cnt->num_mv_found[ref_frame] < MAX_MV_REF_CANDIDATES)
            {
                ...
                if (rf[1] != NONE_FRAME)
                    add_extra_mv_candidate(nbr, parse_ctx, rf,
                        ref_id, ref_id_count, ref_diff, ref_diff_count);
                else
                    process_single_ref_mv_candidate(nbr, parse_ctx, ref_frame,
    ...
    int mi_row, int mi_col, int16_t *mode_context, MvCount *mv_cnt)
{
    ...
    if (ref_frame != INTRA_FRAME) {
        ...
        global_mvs[0].as_int = gm_get_motion_vector(&buf->global_motion[rf[0]],
            parse_ctx->frame_header->allow_high_precision_mv, bsize,
            mi_col, mi_row, parse_ctx->frame_header->force_integer_mv).as_int;

        global_mvs[1].as_int = (rf[1] != NONE_FRAME) ?
            gm_get_motion_vector(&buf->global_motion[rf[1]],
                parse_ctx->frame_header->allow_high_precision_mv, bsize,
                mi_col, mi_row, parse_ctx->frame_header->force_integer_mv).as_int : 0;
    }
    dec_setup_ref_mv_list(dec_handle, parse_ctx, pi, ref_frame, ref_mv_stack, mv_ref_list,
...
/* TODO: Harmonize*/
static void svt_find_best_ref_mvs(int allow_hp, IntMv *mvlist,
...
int dec_assign_dv(ParseCtxt *parse_ctxt, PartitionInfo_t *pi, IntMv *mv,
    IntMv *ref_mv, int mi_row, int mi_col)
{
    SvtReader *r = &parse_ctxt->r;
    ...
    int valid = is_mv_valid(&mv->as_mv) &&
        is_dv_valid(mv->as_mv, parse_ctxt, pi, mi_row, mi_col);
    return valid;
}
    ...
    PartitionInfo_t *pi, int mi_row, int mi_col)
{
    ...
    if (dv_ref.as_int == 0) {
        av1_find_ref_dv(&dv_ref, &parse_ctxt->cur_tile_info,
            parse_ctxt->seq_header->sb_mi_size, mi_row, mi_col);
    }
...
void read_interintra_mode(ParseCtxt *parse_ctxt, BlockModeInfo *mbmi) {
    ...
    if (parse_ctxt->seq_header->enable_interintra_compound
        && !mbmi->skip_mode && is_interintra_allowed(mbmi))
    {
        ...
        if (mbmi->is_inter_intra) {
            ...
            if (is_interintra_wedge_used(bsize)) {
                mbmi->interintra_mode_params.wedge_interintra = svt_read_symbol(
...
static INLINE MotionMode is_motion_mode_allowed(EbDecHandle *dec_handle,
    ...
    int mi_row, int mi_col, int allow_warped_motion)
{
    BlockModeInfo *mbmi = pi->mi;
    if (parse_ctx->frame_header->force_integer_mv == 0) {
    ...
    if ((block_size_wide[mbmi->sb_type] >= 8 && block_size_high[mbmi->sb_type] >= 8) &&
        ...
        if (!has_overlappable_cand(dec_handle, parse_ctx, pi, mi_row, mi_col))
            return SIMPLE_TRANSLATION;
        ...
        {
            if (parse_ctx->frame_header->force_integer_mv) {
                return OBMC_CAUSAL;
            }
    ...
    PartitionInfo_t *pi, int mi_row, int mi_col)
{
    ...
    if (parse_ctxt->frame_header->is_motion_mode_switchable == 0) return SIMPLE_TRANSLATION;
    if (mbmi->skip_mode) return SIMPLE_TRANSLATION;

    const MotionMode last_motion_mode_allowed =
        is_motion_mode_allowed(dec_handle, parse_ctxt,
...
void read_compound_type(EbDecHandle *dec_handle, ParseCtxt *parse_ctxt,
    PartitionInfo_t *pi, int32_t mi_row, int32_t mi_col)
{
    SvtReader *r = &parse_ctxt->r;
    BlockModeInfo *mbmi = pi->mi;
    BlockSize bsize = mbmi->sb_type;
    ...
    if (has_second_ref(mbmi) && !mbmi->skip_mode) {
        ...
        const int masked_compound_used = is_any_masked_compound_used(bsize) &&
            parse_ctxt->seq_header->enable_masked_compound;
        ...
        if (comp_group_idx == 0) {
            if (parse_ctxt->seq_header->order_hint_info.enable_jnt_comp) {
            ...
            assert(parse_ctxt->frame_header->reference_mode != SINGLE_REFERENCE &&
    ...
    update_compound_ctx(parse_ctxt, pi, mi_row, mi_col, comp_group_idx);
}
    ...
    PartitionInfo_t* pi, int mi_row, int mi_col)
{
    BlockModeInfo *mbmi = pi->mi;
    SvtReader *r = &parse_ctxt->r;
    const int allow_hp = parse_ctxt->frame_header->allow_high_precision_mv;
   ...
   /* if ((pi->mi->ref_frame[0] >= BWDREF_FRAME && pi->mi->ref_frame[0] <= ALTREF_FRAME) ||
    ...
    IntMv global_mvs[2];
    av1_find_mv_refs(dec_handle, pi, parse_ctxt, ref_frame, pi->ref_mv_stack,
    ...
    if (!is_compound && mbmi->mode != GLOBALMV) {
        svt_find_best_ref_mvs(allow_hp, ref_mvs[mbmi->ref_frame[0]], &nearestmv[0],
            &nearmv[0], parse_ctxt->frame_header->force_integer_mv);
    }
    if (is_compound && mbmi->mode != GLOBAL_GLOBALMV) {
        ...
        lower_mv_precision(&nearestmv[0].as_mv, allow_hp,
            parse_ctxt->frame_header->force_integer_mv);
    ...
    assign_mv(parse_ctxt, pi, mbmi->mv, global_mvs,
        ref_mv, nearestmv, nearmv, is_compound, allow_hp);
    ...
    pi->num_samples = find_warp_samples(dec_handle, &parse_ctxt->cur_tile_info,
        pi, mi_row, mi_col, pts, pts_inref);

    mbmi->motion_mode = read_motion_mode(dec_handle, parse_ctxt, pi, mi_row, mi_col);

    read_compound_type(dec_handle, parse_ctxt, pi, mi_row, mi_col);

    if (!av1_is_interp_needed(pi, dec_handle->cur_pic_buf[0]->global_motion)) {
        mbmi->interp_filters =
            av1_broadcast_interp_filter(av1_unswitchable_filter
            (parse_ctxt->frame_header->interpolation_filter));
    }
    else {
        if (parse_ctxt->frame_header->interpolation_filter != SWITCHABLE) {
            mbmi->interp_filters =
                av1_broadcast_interp_filter(parse_ctxt->frame_header->interpolation_filter);
        }
        else {
            ...
            for (int dir = 0; dir < 2; ++dir) {
                ...
                if (parse_ctxt->seq_header->enable_dual_filter == 0) {
    ...
    PartitionInfo_t *pi, int mi_row, int mi_col)
{
    ...
    int mi_cols = parse_ctx->frame_header->mi_cols;
    int mi_rows = parse_ctx->frame_header->mi_rows;
    ...
    for (int plane_itr = 0; plane_itr < MAX_MB_PLANE; plane_itr++) {
        ...
        sub_x = plane_itr ? parse_ctx->seq_header->
            color_config.subsampling_x : 0;
        sub_y = plane_itr ? parse_ctx->seq_header->
