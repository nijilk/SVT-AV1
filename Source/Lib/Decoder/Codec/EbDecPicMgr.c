                                ...
                                int32_t refresh_frame_flags)
{
    ...
    if (frame_decoded) {
        for (mask = refresh_frame_flags; mask; mask >>= 1) {
            ...
            dec_handle_ptr->ref_frame_map[ref_index] = dec_handle_ptr->
                                                next_ref_frame_map[ref_index];
...
void generate_next_ref_frame_map(EbDecHandle *dec_handle_ptr) {
    ...
    int32_t ref_index = 0;
    for (int32_t mask = dec_handle_ptr->frame_header.refresh_frame_flags;
            mask; mask >>= 1)
    {
        if (mask & 1)
...
static void set_ref_frame_info(EbDecHandle *dec_handle_ptr, int frame_idx,
                               RefFrameInfo *ref_info)
{
                        ...
                        int32_t gld_map_idx)
{
    ...
    for (int32_t i = 0; i < REF_FRAMES; ++i) {
        ...
        ref_frame_info[i].sort_idx =
            (offset == -1) ? -1
                            : cur_frame_sort_idx +
                get_relative_dist(&dec_handle_ptr->seq_header.order_hint_info,
    ...
    qsort(ref_frame_info, REF_FRAMES, sizeof(RefFrameInfo),
    ...
    set_ref_frame_info(dec_handle_ptr, ALTREF_FRAME - LAST_FRAME,
    ...
    // == BWDREF_FRAME ==
    if (bwd_start_idx <= bwd_end_idx) {
        set_ref_frame_info(dec_handle_ptr, BWDREF_FRAME - LAST_FRAME,
    ...
    // == ALTREF2_FRAME ==
    if (bwd_start_idx <= bwd_end_idx) {
        set_ref_frame_info(dec_handle_ptr, ALTREF2_FRAME - LAST_FRAME,
                            &ref_frame_info[bwd_start_idx]);
    ...
    for (int32_t i = fwd_start_idx; i <= fwd_end_idx; ++i) {
        ...
        if (ref_frame_info[i].map_idx == lst_map_idx) {
            set_ref_frame_info(dec_handle_ptr, LAST_FRAME - LAST_FRAME,
                                &ref_frame_info[i]);
        ...
        if (ref_frame_info[i].map_idx == gld_map_idx) {
            set_ref_frame_info(dec_handle_ptr, GOLDEN_FRAME - LAST_FRAME,
                                &ref_frame_info[i]);
    ...
    for (ref_idx = 0; ref_idx < (INTER_REFS_PER_FRAME - 2); ref_idx++) {
        ...
        set_ref_frame_info(dec_handle_ptr, ref_frame - LAST_FRAME,
