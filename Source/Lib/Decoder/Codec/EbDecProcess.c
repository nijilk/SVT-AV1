                                    ...
                                    TilesInfo *tiles_info)
{
    ...
    ************************************/
    ...
    /* recon top right sync */
    {
        ...
        dec_mt_frame_data->tile_switch_mutex = eb_create_mutex();
        ...
        for (tiles_ctr = 0; tiles_ctr < num_tiles; tiles_ctr++)
        {
            ...
            EB_MALLOC_DEC(int32_t *, dec_mt_frame_data->parse_recon_tile_info_array[tiles_ctr].
                sb_recon_completed_in_row, tile_num_sb_rows * sizeof(int32_t), EB_N_PTR);   

            EB_MALLOC_DEC(int32_t *, dec_mt_frame_data->parse_recon_tile_info_array[tiles_ctr].
                sb_recon_row_started, tile_num_sb_rows * sizeof(int32_t), EB_N_PTR);
                              ...
                              uint32_t tg_start, uint32_t tg_end)
{
    ...
    ParseTileData   *parse_tile_data = master_parse_ctxt->parse_tile_data;
#endif
    int32_t sb_mi_size_log2 = dec_handle_ptr->seq_header.sb_size_log2 -
                                MI_SIZE_LOG2;
#if TILE_GROUP
    int32_t sb_size_h = block_size_high[dec_handle_ptr->seq_header.sb_size];
    ...
    for (uint32_t tile_num = tg_start; tile_num <= tg_end; tile_num++) {
        ...
        int32_t sb_row_start, sb_row_end;
        sb_row_start = tiles_info->tile_row_start_mi[tile_row] >> sb_mi_size_log2;
        sb_row_end = (AOMMIN(tiles_info->tile_row_start_mi[tile_row + 1],
            dec_handle_ptr->frame_header.mi_rows)+ ((1 << sb_mi_size_log2)-1))
            >> sb_mi_size_log2;
        for (int32_t sb_row = sb_row_start; sb_row < sb_row_end; sb_row++) {
            dec_mt_frame_data->sb_recon_row_map[sb_row * tiles_info->tile_cols
                                                + tile_col] = 0;
        }
#endif
        // Assign to ParseCtxt
...
EbErrorType decode_tile_job(EbDecHandle *dec_handle_ptr,
    int32_t tile_num, DecThreadCtxt *thread_ctxt, DecModCtxt *dec_mod_ctxt)
{
    EbErrorType status = EB_ErrorNone;
    TilesInfo   *tiles_info = &dec_handle_ptr->frame_header.tiles_info;
...
void decode_frame_tiles(EbDecHandle *dec_handle_ptr, DecThreadCtxt *thread_ctxt) {
    ...
    while (1) {
        ...
        if (NULL != recon_results_wrapper_ptr) {
            ...
            if (EB_ErrorNone !=
                decode_tile_job(dec_handle_ptr, context_ptr->node_index, thread_ctxt, dec_mod_ctxt))
            {
        ...
        else
        {
            ...
            {
                int32_t tile_num_sb_rows = dec_mt_frame_data->parse_recon_tile_info_array[tile_idx].tile_num_sb_rows;

                //check for incompleted tile with maximum rows to be processed
                if (1 != dec_mt_frame_data->parse_recon_tile_info_array[tile_idx].sb_recon_row_started[tile_num_sb_rows - 1])
                {
                    int32_t ctr;
                    ...
                    {
                        if (0 == dec_mt_frame_data->parse_recon_tile_info_array[tile_idx].sb_recon_row_started[ctr])
                            break;
                    }
            ...
            {
                if (EB_ErrorNone !=
                    decode_tile_job(dec_handle_ptr, next_tile_idx, thread_ctxt,dec_mod_ctxt))
                {
...
static INLINE dec_save_LF_boundary_lines_SB_row(EbDecHandle *dec_handle,
...
{
    int32_t th_cnt = NULL == thread_ctxt ? 0 : thread_ctxt->thread_cnt;
#else
    int32_t th_cnt)
{
#endif
    ...
    while (1) {
        ...
        if (NULL != lf_results_wrapper_ptr) {
            ...
            row_index[2] = (sb_row+(sb_row == (dec_handle->
                master_frame_buf.sb_rows - 1) ? 0 : 1))* tiles_info->tile_cols;
...
void svt_cdef_frame_mt(EbDecHandle *dec_handle_ptr) {
    ...
    while (1) {
        ...
        if (NULL != cdef_results_wrapper_ptr) {
            ...
            int32_t offset = context_ptr->node_index == dec_handle_ptr->
